...
class Symbol:
    ...
    def _validate_matrix_size(self) -> None:
        ...
        if (not isinstance(self.matrix_size, tuple) or 
            len(self.matrix_size) != 2 or
            not all(isinstance(x, int) and x > 0 for x in self.matrix_size)):
    ...
    def calculate_capacity(self) -> int:
        ...
        bits_per_module = {
            4: 2, 8: 3, 16: 4, 32: 5, 64: 6, 128: 7, 256: 8
        }[self.color_count]
