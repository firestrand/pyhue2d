...
def create_matrix(height: int, width: int, fill_value: int = 0, dtype: np.dtype = np.uint8) -> np.ndarray:
...
def extract_submatrix(matrix: np.ndarray, start_row: int, start_col: int, 
    ...
    if (start_row < 0 or start_col < 0 or 
        end_row > matrix.shape[0] or end_col > matrix.shape[1]):
        raise ValueError("Submatrix extends beyond matrix bounds")
    
    return matrix[start_row:end_row, start_col:end_col].copy()


def place_submatrix(matrix: np.ndarray, submatrix: np.ndarray, 
                   start_row: int, start_col: int) -> np.ndarray:
