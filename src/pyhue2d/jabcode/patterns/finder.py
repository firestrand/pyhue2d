...
class FinderPatternGenerator:
    ...
    def generate_finder_pattern_layers(
        self, symbol_width: int, symbol_height: int, is_master: bool = True
    ) -> np.ndarray:
            ...
            for i in range(k + 1):
                for j in range(k + 1):
                    ...
                    if i == k or j == k:
                        ...
                        self._place_corner_modules(
                            matrix, symbol_width, symbol_height, i, j, fp0_color, fp1_color, fp2_color, fp3_color
                        )

        return matrix

    def _place_corner_modules(
        self,
        matrix: np.ndarray,
        width: int,
        height: int,
    ...
    def extract_finder_pattern(
        self, matrix: np.ndarray, pattern_type: int, symbol_width: int, symbol_height: int
    ) -> np.ndarray:
