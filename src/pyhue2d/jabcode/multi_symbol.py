...
from .core import Bitmap, EncodedData
...
class MultiSymbolCascade:
    ...
    def encode_to_bitmaps(self, data: Union[str, bytes]) -> List[Bitmap]:
        ...
        for encoded_symbol in encoded_symbols:
            ...
            bitmap = self.encoder.encode_to_bitmap(
                encoded_symbol.data[:100] if len(encoded_symbol.data) > 100 else encoded_symbol.data
            )
    ...
    def _split_data(self, data: bytes) -> List[bytes]:
        ...
        while offset < len(data):
            ...
            if len(segments) >= self.max_symbols:
                if offset < len(data):
                    raise ValueError(f"Data too large: requires more than {self.max_symbols} symbols")
                break
    ...
    def get_encoding_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_symbols_per_encode": (self._stats["total_symbols_created"] / self._stats["total_encodes"]),
