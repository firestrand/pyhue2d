...
from .constants import DEFAULT_4_COLOR_PALETTE, DEFAULT_8_COLOR_PALETTE
...
class ModuleDataExtractor:
    ...
    def extract_module_data(self, sampled_symbol: np.ndarray, symbol_metadata: Dict[str, Any]) -> List[int]:
        ...
        color_indices = self._decode_module_colors(sampled_symbol, color_palettes, color_count)
        ...
        if self.settings["demasking_enabled"]:
            data_indices = self._demask_data(data_indices, mask_pattern, color_count, symbol_metadata)

        self._stats["modules_processed"] += len(data_indices)
        return data_indices

    def _read_color_palettes(self, sampled_symbol: np.ndarray, metadata: Dict[str, Any]) -> List[np.ndarray]:
    ...
    def _read_master_color_palettes(self, sampled_symbol: np.ndarray, color_count: int) -> List[np.ndarray]:
    ...
    def _read_slave_color_palettes(self, sampled_symbol: np.ndarray, color_count: int) -> List[np.ndarray]:
    ...
    def _extract_palette_from_region(self, region: np.ndarray, color_count: int) -> np.ndarray:
    ...
    def _interpolate_color_palettes(self, palettes: List[np.ndarray], color_count: int) -> List[np.ndarray]:
    ...
    def _perform_color_interpolation(self, base_palette: np.ndarray, target_count: int) -> np.ndarray:
        ...
        for i in range(target_count):
            ...
            if lower_idx == upper_idx:
                ...
                interpolated[i] = ((1 - weight) * base_palette[lower_idx] + weight * base_palette[upper_idx]).astype(
                    np.uint8
                )

        return interpolated

    def _create_data_map(self, width: int, height: int, metadata: Dict[str, Any]) -> np.ndarray:
    ...
    def _decode_module_colors(
        self,
        sampled_symbol: np.ndarray,
    ...
    def _decode_color_hard_decision(self, rgb: np.ndarray, palette: np.ndarray) -> int:
    ...
    def _decode_color_soft_decision(self, rgb: np.ndarray, palette: np.ndarray) -> int:
    ...
    def _filter_data_modules(self, color_indices: np.ndarray, data_map: np.ndarray) -> List[int]:
