...
class PerspectiveTransformer:
    ...
    def square_to_quad(self, quad_points: List[Point2D]) -> PerspectiveTransform:
        ...
        if abs(dx3) < 1e-10 and abs(dy3) < 1e-10:
            ...
            matrix = np.array([[x1 - x0, x3 - x0, x0], [y1 - y0, y3 - y0, y0], [0.0, 0.0, 1.0]])
        else:
    ...
    def get_jabcode_perspective_transform(
        self, finder_patterns: List[Point2D], symbol_size: Tuple[int, int]
    ...
    def warp_points(self, transform: PerspectiveTransform, points: Union[List[Point2D], np.ndarray]) -> List[Point2D]:
    ...
    def sample_symbol_with_perspective(
        self,
        image: Image.Image,
