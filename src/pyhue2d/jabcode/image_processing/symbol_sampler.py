...
class SymbolSampler:
    ...
    def sample_symbol(
        ...
        """
        return self.perspective_transformer.sample_symbol_with_perspective(image, perspective_transform, symbol_size)

    def sample_modules_at_positions(self, image: Image.Image, positions: List[Point2D]) -> List[int]:
    ...
    def sample_with_interpolation(self, image: Image.Image, positions: List[Point2D]) -> List[float]:
    ...
    def _bilinear_interpolation(
        ...
        value = p00 * (1 - wx) * (1 - wy) + p10 * wx * (1 - wy) + p01 * (1 - wx) * wy + p11 * wx * wy
    ...
    def extract_symbol_region(
        ...
        transform = self.perspective_transformer.get_jabcode_perspective_transform(finder_patterns, symbol_size)
