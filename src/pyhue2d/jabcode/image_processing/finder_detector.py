...
class FinderPatternDetector:
    ...
    def __init__(self, settings: Optional[Dict[str, Any]] = None):
        ...
        self.detection_methods = ['template_matching', 'contour_detection', 'hybrid', 'scanline']
    ...
    def validate_pattern(self, pattern_region: np.ndarray, center: Point2D) -> bool:
        ...
        try:
            ...
            try:
                ...
                if noise_level > 70:  # Moderate threshold - JABCode patterns can have structured edges, but reject pure noise
                    return False
            except ImportError:
    ...
    def assess_pattern_quality(self, pattern_array: np.ndarray) -> float:
        ...
        try:
            ...
            size_score = 1.0 if self.settings['min_pattern_size'] <= min(h, w) <= self.settings['max_pattern_size'] else 0.5
    ...
    def _create_templates(self) -> None:
        ...
        # JABCode spec: use only odd sizes
        sizes = [15, 21, 29, 31]
        pattern_types = [FinderPatternType.FP0, FinderPatternType.FP1, FinderPatternType.FP2, FinderPatternType.FP3]
        for size in sizes:
            for pattern_type in pattern_types:
                template = generator.generate_pattern(pattern_type, size=size) * 255  # 0/1 to 0/255
                self.templates[f"FP{pattern_type}_{size}"] = template
    ...
    def _detect_by_scanline(self, array: np.ndarray) -> List[Dict[str, Any]]:
        ...
        def check_pattern_cross(lengths):
            ...
            if len(lengths) >= 3:
                ...
                if len(lengths) == 5:
                    ...
                    valid = (abs(layer_size - lengths[1]) < layer_tolerance and
                             abs(layer_size - lengths[2]) < layer_tolerance and
                             abs(layer_size - lengths[3]) < layer_tolerance and
                             lengths[0] > 0.5 * layer_tolerance and
                             lengths[4] > 0.5 * layer_tolerance and
                             abs(lengths[1] - lengths[3]) < layer_tolerance)
                else:
                    ...
                    if valid:
                        ...
                        if len(lengths) >= 3:
                            ...
                            if len(inner_layers) > 1:
                                ...
                                consistency_score = max(0.1, 1.0 - (layer_variance / (module_size * module_size)))
                            else:
                            ...
                            size_score = 1.0 if self.settings['min_pattern_size'] <= pattern_width <= self.settings['max_pattern_size'] else 0.3
                        ...
                        if (confidence >= 0.5 and  # Reasonable confidence threshold
                            self.settings['min_pattern_size'] <= pattern_size <= self.settings['max_pattern_size']):
        ...
        if patterns:
            ...
            for pattern in patterns:
                ...
                if (size_diff <= threshold and 
                    pattern['confidence'] >= self.settings.get('quality_threshold', 0.5)):
    ...
    def _detect_by_contour_detection(self, gray: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for contour in contours:
            ...
            if not (1 - self.settings['aspect_ratio_tolerance'] <= aspect_ratio <= 1 + self.settings['aspect_ratio_tolerance']):
                continue
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_detection_time': (
                self._stats['total_detection_time'] / self._stats['total_detections']
            ),
            ...
            'avg_patterns_per_detection': (
                self._stats['patterns_found'] / self._stats['total_detections']
            ),
        }
