...
class TestImageExporterImplementation:
    ...
    def test_exporter_quality_settings_jpeg(self, exporter, test_bitmap):
        ...
        for quality in quality_levels:
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp_file:
                try:
                    exporter.export_bitmap(test_bitmap, tmp_file.name, format="JPEG", quality=quality)
    ...
    def test_exporter_dpi_settings(self, exporter, test_bitmap):
        ...
        for dpi in dpi_values:
            with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_file:
                try:
                    exporter.export_bitmap(test_bitmap, tmp_file.name, format="PNG", dpi=dpi)
    ...
    def test_exporter_batch_export(self, exporter):
        ...
        if hasattr(exporter, "export_batch"):
            with tempfile.TemporaryDirectory() as tmp_dir:
                filenames = [os.path.join(tmp_dir, f"test_{i}.png") for i in range(len(bitmaps))]
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_file:
            try:
                if hasattr(exporter, "export_bitmap_with_metadata"):
                    exporter.export_bitmap_with_metadata(test_bitmap, tmp_file.name, metadata=metadata)
                else:
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_file:
            try:
                with pytest.raises(ValueError):
                    exporter.export_bitmap(test_bitmap, tmp_file.name, format="INVALID_FORMAT")
            finally:
                if os.path.exists(tmp_file.name):
                    os.unlink(tmp_file.name)
